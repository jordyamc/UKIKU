...
class HomeFragmentMaterial : BottomFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.listNew.apply {
            setAdapter(RecentsAV1Adapter(this@HomeFragmentMaterial, isMaterial = true, isLarge = false, showSeen = false))
            setViewAllOnClick {
        ...
        binding.listFavUpdated.apply {
            setAdapter(RecentsAV1Adapter(this@HomeFragmentMaterial, true))
            setViewAllClass(RecentsModelActivity::class.java)
        }
        binding.listBestEmission.setAdapter(DirAdapter(this, true))
        ...
        binding.listWaiting.apply {
            setAdapter(WaitingAdapter(this@HomeFragmentMaterial))
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            CacheDB.INSTANCE.favoriteAV1DAO().countFlow.drop(1).collect {
                val cached = CacheDB.INSTANCE.recentAV1DAO().all
                val filtered = cached.filter {
                    CacheDB.INSTANCE.favoriteAV1DAO().isFav(it.aid)
                }
