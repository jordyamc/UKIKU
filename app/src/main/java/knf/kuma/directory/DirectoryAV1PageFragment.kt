...
class DirectoryAV1PageFragment : BottomFragment() {
    lateinit var recyclerView: RecyclerView
    lateinit var progress: ProgressBar
    private var manager: RecyclerView.LayoutManager? = null
    private val adapter: DirectoryAV1PageAdapter by lazy { DirectoryAV1PageAdapter(requireActivity()) }
    ...
    fun getType() =
            when (arguments?.getInt("type", 0)) {
                1 -> "&category=ova&category=especial"
                2 -> "&category=pelicula"
                else -> "&category=tv-anime"
            }
    ...
    companion object {
        ...
        operator fun get(type: Int): DirectoryAV1PageFragment {
            val bundle = Bundle()
            bundle.putInt("type", type)
            val fragment = DirectoryAV1PageFragment()
