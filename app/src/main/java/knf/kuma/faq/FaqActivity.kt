...
class FaqActivity : GenericActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        MaterialDialog(this).safeShow {
            ...
            listItems(items = listOf("Telegram", "Email")) { _, index, _ ->
                when (index) {
                    0 -> startActivity(Intent(Intent.ACTION_VIEW, "https://t.me/ukiku_group".toUri()))
                    1 -> {
    ...
    private fun createFAQList(): List<FaqItem> =
            listOf(
                    ...
                    FaqItem("¿Por qué no está este anime en la app?", "La app utiliza AnimeAV1 para obtener la lista de animes, si no te aparece es porque, o no esta en Animeflv o no estas escribiendo el nombre correctamente"),
                    FaqItem("¿Puedo pedir animes?", "No, la app utiliza AnimeAV1, no subimos los animes"),
                    ...
                    FaqItem("¿Que significa Cloudflare activado?", "Es una protección de la página de AnimeAV1, la app debería poder pasar esa protección automáticamente"),
