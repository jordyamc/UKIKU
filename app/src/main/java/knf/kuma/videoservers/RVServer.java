    ...
    public VideoServer getVideoServer() {
        try {
            ...
            if (down_link.contains("&server=rv"))
                down_link = PatternUtil.getRapidLink(Jsoup.connect(down_link).cookies(BypassUtil.getMapCookie(context)).userAgent(BypassUtil.userAgent).get().outerHtml()).replace("&q=720p", "");
            VideoServer videoServer = new VideoServer(RV);
            try {
                String jsoup720 = Jsoup.connect(down_link + "&q=720p").cookies(BypassUtil.getMapCookie(context)).userAgent(BypassUtil.userAgent).get().select("video source").first().attr("src");
                videoServer.addOption(new Option(getName(), "720p", jsoup720));
            } catch (Exception e) {
                //
            }
            try {
                String jsoup480 = Jsoup.connect(down_link + "&q=480p").cookies(BypassUtil.getMapCookie(context)).userAgent(BypassUtil.userAgent).get().select("video source").first().attr("src");
