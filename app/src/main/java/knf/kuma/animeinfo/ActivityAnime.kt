...
import knf.kuma.animeinfo.ActivityAnimeMaterial.Companion.keyMaterial
...
class ActivityAnime : GenericActivity(), AnimeActivityHolder.Interface {
    ...
    private val isMaterial get() = intent?.getBooleanExtra(keyMaterial, false) ?: false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (aidOnly)
            viewModel.init(intent.getIntExtra(keyAidOnly, -1))
        else
    ...
    companion object {
        ...
        fun open(fragment: Fragment, recentObject: RecentAV1, view: ImageView, persist: Boolean = true, animate: Boolean = true) {
            val activity = fragment.activity ?: return
            val intent = Intent(fragment.context, DesignUtils.infoClass)
            intent.data = recentObject.animeUrl.toUri()
            intent.putExtra(keyTitle, recentObject.name)
            intent.putExtra(keyAid, recentObject.aid)
            intent.putExtra(keyImg, recentObject.animeImageUrl)
        ...
        fun open(fragment: Fragment, item: DirectoryAV1Calendar, view: ImageView, persist: Boolean = true, animate: Boolean = true) {
            val activity = fragment.activity ?: return
            val intent = Intent(fragment.context, DesignUtils.infoClass)
            intent.data = item.animeUrl.toUri()
            intent.putExtra(keyTitle, item.name)
            intent.putExtra(keyAid, item.aid)
            intent.putExtra(keyImg, item.imageUrl)
            intent.putExtra(keyPersist, persist)
            intent.putExtra(keyNoTransition, !animate)
            fragment.startActivity(intent, ActivityOptionsCompat.makeSceneTransitionAnimation(activity, view, sharedImg).toBundle())
        }

        fun open(activity: Activity, animeObject: Recommended, view: ImageView, persist: Boolean, animate: Boolean) {
        ...
        fun open(activity: Activity, animeObject: DirectoryAV1Min, view: ImageView, persist: Boolean, animate: Boolean) {
            val intent = Intent(activity, DesignUtils.infoClass)
            intent.data = animeObject.animeUrl.toUri()
            intent.putExtra(keyTitle, animeObject.name)
            intent.putExtra(keyAid, animeObject.aid)
        ...
        fun getSimpleIntent(item: DirectoryAV1Calendar): Intent {
            val intent = Intent()
            intent.data = item.animeUrl.toUri()
            intent.putExtra(keyTitle, item.name)
            intent.putExtra(keyAid, item.aid)
