...
class AnimeChaptersMaterialHolder(view: View, private val fragment: Fragment, private val callback: ChapHolderCallback) {
    ...
    fun setAdapter(fragment: Fragment, anime: DirectoryAV1, chapters: MutableList<Chapter>) {
        ...
        fragment.viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO){
            ...
            this@AnimeChaptersMaterialHolder.adapter = AnimeChaptersAdapterMaterial(fragment, recyclerView, anime, chapters, touchListener, true)
            recyclerView.post {
    ...
    fun goToChapter() {
        fragment.lifecycleScope.launch(Dispatchers.IO){
            if (chapters.isNotEmpty()) {
                val eids = chapters.sortedBy { it.number }.map { it.eid }
